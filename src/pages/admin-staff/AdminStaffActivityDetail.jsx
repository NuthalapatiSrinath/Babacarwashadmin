...
/* ═══════════════════════════════════════════════════════════════════════════
   MAIN COMPONENT
═══════════════════════════════════════════════════════════════════════════ */
const AdminStaffActivityDetail = () => {
  ...
  const fetchDetail = useCallback(
    async (silent = false) => {
      ...
      try {
        ...
        if (selectedDeviceId) {
          qs += `&deviceId=${encodeURIComponent(selectedDeviceId)}`;
          qsAll += `&deviceId=${encodeURIComponent(selectedDeviceId)}`;
        }
        const [mainRes, allRes] = await Promise.all([
          api.get(`/admin-activities/admin/${adminId}?${qs}`),
          api.get(`/admin-activities/admin/${adminId}?${qsAll}`),
        ]);
        ...
        toast.error(
          err.response?.data?.message || "Failed to load admin staff activity",
        );
      } finally {
        setLoading(false);
      }
  ...
  const formSubmissions = useMemo(
    () => allActivities.filter((a) => ["form_submit"].includes(a.activityType)),
    [allActivities],
  );
  ...
  const crudActivities = useMemo(
    () =>
      allActivities.filter((a) =>
        ["crud_create", "crud_update", "crud_delete", "crud_read"].includes(
          a.activityType,
        ),
      ),
    [allActivities],
  ...
  const searchFilterActivities = useMemo(
    () =>
      allActivities.filter((a) =>
        ["search_filter", "search"].includes(a.activityType),
      ),
    [allActivities],
  );
  ...
  const pageAnalytics = useMemo(() => {
    ...
    allActivities.forEach((a) => {
      ...
      if (["page_view", "screen_time"].includes(a.activityType)) {
        map[pagePath].visits++;
      }
  ...
  const loginHistory = useMemo(
    () =>
      allActivities
        .filter((a) => ["login", "logout"].includes(a.activityType))
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-purple-50/30 to-blue-50/20">
        <div className="text-center">
          ...
          <p className="text-slate-500">Loading admin staff analytics…</p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600">Admin staff data not found.</p>
          <button
            onClick={() => navigate(-1)}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50/20 to-blue-50/10 p-4 md:p-6 space-y-5">
      ...
      >
        ...
        >
          <ArrowLeft className="w-4 h-4" /> Back to Admin Staff
        </button>
            ...
            <div className="flex items-center gap-5">
              ...
              <div>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="flex items-center gap-1 text-xs text-indigo-700 bg-indigo-100 border border-indigo-200 px-2.5 py-0.5 rounded-full font-medium">
            ...
            {devices.map((dev) => {
              ...
              return (
                <button
                  key={dev.deviceId}
                  onClick={() => {
                    setSelectedDeviceId(isSelected ? null : dev.deviceId);
                    setPage(1);
                  }}
          ...
          {selectedDevice && (
            <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 mt-2">
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 text-xs">
                <div>
                  <span className="text-violet-400 block">Browser</span>
                  <span className="font-bold text-slate-700">
                    {selectedDevice.browser || "—"}
                  </span>
                </div>
                <div>
                  <span className="text-violet-400 block">OS</span>
                  <span className="font-bold text-slate-700">
                    {selectedDevice.os || selectedDevice.platform || "—"}
                  </span>
                </div>
                <div>
                  <span className="text-violet-400 block">Type</span>
                  <span className="font-bold text-slate-700">
                    {selectedDevice.deviceType || "—"}
                  </span>
                </div>
                <div>
                  <span className="text-violet-400 block">Sessions</span>
                  <span className="font-bold text-slate-700">
                    {selectedDevice.sessionCount || 0}
                  </span>
                </div>
                <div>
                  <span className="text-violet-400 block">Last Seen</span>
                  <span className="font-bold text-slate-700">
                    {fmtDate(selectedDevice.lastSeen)}
                  </span>
                </div>
                <div>
                  <span className="text-violet-400 block">Resolution</span>
                  <span className="font-bold text-slate-700">
      ...
      {devices && devices.length > 1 && !selectedDeviceId && (
        <Card delay={0.07}>
          <SectionHeader
            icon={BarChart2}
            title="Device-wise Stats Comparison"
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="text-slate-400 text-left border-b border-slate-100">
                  ...
                  <th className="pb-2 pr-3 font-semibold text-right">Events</th>
                  <th className="pb-2 pr-3 font-semibold text-right">
                    Sessions
                  </th>
                  <th className="pb-2 pr-3 font-semibold text-right">Pages</th>
                  <th className="pb-2 pr-3 font-semibold text-right">Clicks</th>
                  <th className="pb-2 pr-3 font-semibold text-right">Logins</th>
                  <th className="pb-2 pr-3 font-semibold text-right">
                    Screen Time
                  </th>
              ...
              <tbody>
                {devices.map((dev, i) => (
                  ...
                  >
                    ...
                    <td className="py-2.5 pr-3 text-right font-bold text-slate-600">
                      {dev.totalActivities}
                    </td>
                    ...
                    <td className="py-2.5 pr-3 text-right text-slate-600">
                      {fmt(Math.round((dev.totalDuration || 0) / 1000))}
                    </td>
      ...
      {/* ── 7. Admin Staff Actions (CRUD) ─────────────────────────────────── */}
      <Card delay={0.25}>
        <SectionHeader
          icon={Settings}
          title="Admin Staff Actions (CRUD)"
          color="text-sky-500"
        ...
        {crudActivities.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-xs text-slate-400">
              Create, update, delete actions will appear here
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
      ...
      {/* ── 8. Form Submissions & Search/Filter ──────────────────────────── */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Card delay={0.27}>
          <SectionHeader
            icon={FileText}
          ...
          {formSubmissions.length === 0 ? (
            <EmptyState text="No form submissions recorded" />
          ) : (
            <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
              {formSubmissions.map((a, i) => (
                <div
                  key={a._id || i}
                  className="border border-slate-100 rounded-xl p-3 hover:border-amber-200 transition-colors bg-slate-50/40"
                >
        ...
        <Card delay={0.29}>
          <SectionHeader
            icon={Search}
            title="Search & Filter Activity"
            color="text-slate-500"
            badge={searchFilterActivities.length}
          />
          {searchFilterActivities.length === 0 ? (
            <EmptyState text="No search/filter activity recorded" />
          ) : (
            <div className="space-y-3 max-h-96 overflow-y-auto pr-1">
              {searchFilterActivities.map((a, i) => (
                <div
                  key={a._id || i}
                  className="border border-slate-100 rounded-xl p-3 hover:border-slate-200 transition-colors bg-slate-50/40"
                >
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <ActivityBadge type={a.activityType} />
                    <span className="text-xs text-slate-400">
      ...
      <Card delay={0.3}>
        ...
        {loginHistory.length === 0 ? (
          <EmptyState text="No login/logout events" />
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
      ...
      <Card delay={0.32}>
        <div className="bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-pink-500/10 -m-5 p-5 rounded-t-2xl border-b-2 border-indigo-200 mb-4">
          ...
          />
          <p className="text-xs text-slate-600 mt-1 font-medium">
            Complete admin staff journey from login to logout — with detailed
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div id={`adm-session-${i}`} className="space-y-2">
          ...
          {deviceInfo ? (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                ...
                {(deviceInfo.device?.os || deviceInfo.device?.platform) && (
                  <div className="bg-slate-50 rounded-xl p-2.5">
                    <span className="text-slate-400 block">OS / Platform</span>
                    <span className="font-semibold text-slate-700">
                      {deviceInfo.device.os || deviceInfo.device.platform}
                    </span>
                  </div>
                )}
                {deviceInfo.device?.screenResolution && (
                  <div className="bg-slate-50 rounded-xl p-2.5">
      ...
      <Card delay={0.38} className="overflow-hidden">
...
export default AdminStaffActivityDetail;
