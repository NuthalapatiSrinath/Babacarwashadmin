...
const SupervisorYearlyRecords = () => {
  const navigate = useNavigate();
  const location = useLocation();

  // Restore state from sessionStorage if available
  const savedState = sessionStorage.getItem("supervisorYearlyRecordsState");
  ...
  // Fetch only the supervisor's assigned team members
  useEffect(() => {
    const fetchTeamWorkers = async () => {
      try {
        ...
        if (response.data) setAllWorkers(response.data);
      } catch (e) {
        console.error("Failed to fetch team workers:", e);
  ...
  const handleViewSlip = (monthIndex) => {
    ...
    navigate(url, {
      state: {
        from: "/supervisor/yearly-records",
...
export default SupervisorYearlyRecords;
