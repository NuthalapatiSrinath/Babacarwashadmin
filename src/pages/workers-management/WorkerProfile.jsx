...
const WorkerProfile = () => {
  ...
  const fetchWorker = async () => {
    try {
      const res = await workerService.info(id);
      if (res.data) {
        setWorker(res.data);
      } else {
  ...
  const handleProfileImageChange = async (e) => {
    ...
    try {
      await workerService.uploadProfileImage(id, file);
  ...
  const handleDocUpload = async (file, type) => {
    ...
    try {
      await workerService.uploadDocument(id, file, type);
          ...
          <div className="p-6 lg:p-8 flex flex-col items-center w-full">
            ...
            <div className="text-center mb-6 lg:mb-8">
              ...
              <span className="text-xs lg:text-sm font-bold text-slate-500 uppercase tracking-widest">
                {worker.companyName || "WORKER"}
              </span>
            </div>

            {/* 3. ✅ UNIFIED INFO CARD (Dark Theme) */}
          ...
          <div className="p-6 lg:p-12 flex flex-col justify-center min-h-full max-w-4xl mx-auto">
            ...
            <div className="space-y-4 lg:space-y-6">
              ...
              <SkillBox
                ...
                hasDoc={!!worker.passportDocument?.url}
                docUrl={worker.passportDocument?.url}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-white text-base lg:text-lg flex items-center gap-2">
                      {formatDate(worker.passportExpiry)}
                      {isExpired(worker.passportExpiry) && (
              ...
              <SkillBox
                ...
                hasDoc={!!worker.visaDocument?.url}
                docUrl={worker.visaDocument?.url}
              ...
              <SkillBox
                number="03"
                title="EMIRATES ID"
                hasDoc={!!worker.emiratesIdDocument?.url}
                docUrl={worker.emiratesIdDocument?.url}
              ...
              >
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <span className="font-bold text-white text-base lg:text-lg">
                      {formatDate(worker.emiratesIdExpiry)}
                    </span>
                  </div>
                </div>
              </SkillBox>
...
// ✅ Reusable Blue Gradient Box (Identical to StaffProfile)
